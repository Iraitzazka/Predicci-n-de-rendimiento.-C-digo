with open("2020nombres.txt", "r", encoding = "utf-8") as datos1:
    listaval = []
    for linea in datos1:
            listaval.append(linea[:-1]) #Me quito el salto de linea

with open("2019-2020nombres_normalizados(sobran).txt", "r",encoding = "utf-8") as datos2:
    listastats = []
    for linea in datos2:
...
listaval.sort()
listastats.sort()
with open('2020nombres_ast.txt','w',encoding = "utf-8") as datos:
    for i in listaval:
        datos.write(i + '\n')
with open('2019-2020nombres_normalizados(sobran).txt','w',encoding = "utf-8") as datos:
    for i in listastats:
        datos.write(i + '\n')
